...
// ─── Safe number helpers ───────────────────────────────────────────────────
/** Converts any value to a finite number. Returns fallback if result is NaN/Infinite. */
...
// ─── Side badge ─────────────────────────────────────────────────────────────
// ask_side_ratio: 0.0–1.0  (fraction of volume that traded AT or ABOVE the ask)
...
// all_opening_trades: explicit UW boolean
// volume_oi_ratio: volume / OI  (>1 = impossible without new positions)
...
function PremiumBar({ askPremium, bidPremium, total }: { askPremium?: number | null; bidPremium?: number | null; total: number }) {
  if (!askPremium && !bidPremium) {
...
// ═══════════════════════════════════════════════════════════════════════════
interface FlowAlert {
  id?: string; ticker?: string; strike?: string | number; put_call?: string;
  expiry?: string | null; total_premium?: number | null; premium?: number | null;
  ask_premium?: number | null; bid_premium?: number | null;
  ask_side_ratio?: number | null; all_opening_trades?: boolean | null;
  volume_oi_ratio?: number | null; size?: number | null;
  open_interest?: number | null; iv_start?: number | null; iv?: number | null;
  has_sweep?: boolean; is_sweep?: boolean; is_golden_sweep?: boolean;
}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              ...
              {flows.map((f, i) => {
                ...
                const volOi    = f.volume_oi_ratio ?? null;
                return (
                  <tr key={f.id ?? i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.015)" }}>
                    ...
                    <td style={{ padding: "9px 10px" }}><Badge color={isCall ? "green" : "red"}>{isCall ? "CALL" : "PUT"}</Badge></td>
                    ...
                    <td style={{ padding: "9px 10px" }}>
                      <PremiumBar total={premium} askPremium={f.ask_premium} bidPremium={f.bid_premium} />
                    </td>
                    <td style={{ padding: "9px 10px", color: "#e2e8f0", fontFamily: "monospace" }}>{f.size?.toLocaleString() ?? "—"}</td>
                    ...
                    <td style={{ padding: "9px 10px" }}><SideBadge ratio={f.ask_side_ratio} /></td>
                    <td style={{ padding: "9px 10px" }}><OpenCloseBadge allOpening={f.all_opening_trades} volOiRatio={volOi} /></td>
                    <td style={{ padding: "9px 10px" }}>
                      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                        {f.is_golden_sweep && <Badge color="amber">GOLDEN ★</Badge>}
    ...
    try {
      ...
      for (const ticker of WATCHLIST) {
        try {
          ...
          const iv     = safeNum(d["implied-volatility-30-day"]   ?? d.iv,     0);
          const hv     = safeNum(d["historical-volatility-30-day"] ?? d.hv,     0);
          const ivRank = safeNum(d["iv-rank"]                      ?? d.ivRank, 50);
...
function AccountTab() {
  ...
  const netLiq   = balances?.["net-liquidating-value"]     ?? balances?.net_liq      ?? null;
  const buyPower = balances?.["derivative-buying-power"]   ?? balances?.buying_power  ?? null;
  const cashBal  = balances?.["cash-balance"]              ?? balances?.cash           ?? null;
      ...
      {!loading && !error && (
        <>
          <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
            {[
              { label: "NET LIQ",      value: fmtBal(netLiq),   color: "#10b981" },
              { label: "BUYING POWER", value: fmtBal(buyPower), color: "#06b6d4" },
              { label: "CASH",         value: fmtBal(cashBal),  color: "#e2e8f0" },
...
// ═══════════════════════════════════════════════════════════════════════════
// KELLY LAB  — fixed: clamp f to 0.999 max to avoid log(0) = -Infinity crash
// ═══════════════════════════════════════════════════════════════════════════
function KellyTab() {
  ...
  // ★ FIX: clamp f to [0, 0.999] — f=1 causes log(1-1)=log(0)=-Infinity which crashes .toFixed()
  const points = Array.from({ length: 20 }, (_, i) => {
    const f   = Math.min(i / 19, 0.999);
    const raw = b > 0 ? p * Math.log(1 + b * f) + q * Math.log(1 - f) : 0;
    const g   = isFinite(raw) ? raw * 100 : 0;
    return { f: Math.round(f * 100), g };
  });
...
interface ChainLeg { symbol?: string; delta?: string; bid?: string; ask?: string; iv?: string; }
...
function ChainTab() {
  ...
  const loadExpiries = useCallback(async (sym: string) => {
    ...
    try {
      ...
      setExpiries(list);
      setSelExp(list[0]?.expiration_date ?? list[0]?.["expiration-date"] ?? "");
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }, []);
...
export default function OptionsEdgeScanner() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0b0f1a", color: "#e2e8f0", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      {/* Global keyframes — single style tag, no hydration mismatch */}
      <style>{`
